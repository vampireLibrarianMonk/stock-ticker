        ...
        if form.is_valid():
            ...
            if response.status_code == 200:
                ...
                if stock_data:  # Checks if the API response contains data.
                    ...
                    messages.error(request, f"{ticker_symbol} is valid but returned no data. Please check the ticker symbol.")
            else:
...
def portfolio_management(request):
    ...
    for ticker_item in ticker:
        try:
            ...
            api_return = json_loads(api_request.content)[0]  # Assuming the API returns a list
